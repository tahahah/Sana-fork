    ...
    def run_sampling(init_z=None, label_suffix="", vae=None, sampler="dpm-solver"):
        ...
        for idx, latent in enumerate(latents):
            ...
            if accelerator.is_main_process and idx == 0: # Log for the first item in batch only, for now
                ...
                num_total_raw_frames = len(raw_frames)
                ...
                # Send to accelerator.log
                if wandb_images_to_log:
                    logger.info(f"[DEBUG run_sampling] Logging val/detailed_sequence{label_suffix} with {len(wandb_images_to_log)} images")
                    accelerator.log({f"val/detailed_sequence{label_suffix}": wandb_images_to_log}, step=step)
                    logger.info(f"[DEBUG run_sampling] Logged val/detailed_sequence{label_suffix}")
                else:
                    logger.info(f"[DEBUG run_sampling] No images to log for detailed sequence.")
