...
def load_png_as_tensor(png_path, resolution=512):
    """Load a PNG file and convert it to a tensor.
    
    Args:
        png_path: Path to the PNG file
        resolution: Target resolution for the image (default: 512)
    
    Returns:
        torch.Tensor: Image tensor in [C, H, W] format, normalized to [0, 1]
    """
    # Define transforms
...
def load_image_as_tensor(png_path, resolution=512):
    """Load a PNG file and convert it to a tensor.
    ...
    transform = transforms.Compose([
                ...
                transforms.Resize(resolution),          # Resize to 512x512
...
def visualize(config, args, model, items, bs, sample_steps, cfg_scale, pag_scale=1.0):
    ...
    for chunk in tqdm(list(get_chunks(items, bs)), desc=tqdm_desc, unit="batch", position=args.gpu_id, leave=True):
        # init frame
        actions, obs, x = (
            torch.tensor([[0, 0, 0, 0, 1]]).unsqueeze(0).repeat(1, 1, (seq_len-1), 1), # [b, 1, S, A]
            torch.cat(torch.zeros([bs, config.vae.vae_latent_dim*(seq_len-1), latent_size, latent_size], device=device), load_image_as_tensor("image.jpg")), # latent noise, # Latent observations
            torch.randn(bs, config.vae.vae_latent_dim, latent_size, latent_size, device=device, generator=generator), # latent noise
        )
        if bs == 1:
        ...
        # Add the current frame to obs for next frame prediction
        obs = torch.cat((obs[:, :-config.vae.latent_dim, :, :], samples), dim=1)


        # Add the current frame to obs for next frame prediction
        ## Only decode to show to user
        samples = vae_decode(config.vae.vae_type, vae, samples)

        
        # Add next action from user
        actions = torch.cat(
            (
                actions[:, :, :-1, :],
                torch.tensor(action_input).unsqueeze(0).repeat(1, 1, (seq_len-1), 1),
            ),
            dim=1,
        )
...
if __name__ == "__main__":
    ...
    print(
        colored(f"Pacman inference has finished. Results stored at ", "green"),
