...
class PacmanDataset(IterableDataset):
    def __init__(
        ...
        initial_offset=0,   # Number of initial samples to skip
        config=None,
        vae=None,  
        **kwargs,
    ):
        ...
        self._offset_applied = False  # Track if initial offset has been applied
        self._dataset_iterator = None  # Dataset iterator for streaming
    ...
    def _fill_buffer(self):
        ...
        # Initialize dataset iterator if not already done
        if self._dataset_iterator is None:
            self._dataset_iterator = iter(self.dataset)
            
        # Apply initial offset if not already done
        if not self._offset_applied and self.initial_offset > 0:
            try:
                for _ in range(self.initial_offset):
                    next(self._dataset_iterator)
                self._offset_applied = True
                self.logger.info(f"Skipped {self.initial_offset} initial samples")
            except StopIteration:
                self.logger.warning(f"Reached end of dataset while applying initial offset")
                self._offset_applied = True
                return
        ...
        try:
            ...
            samples = list(islice(self._dataset_iterator, batch_size))
    ...
    def _process_sequences(self):
        ...
        while not self._stop_event.is_set():
            try:
                ...
                while self.sample_buffer and not self._stop_event.is_set():
                    # Get the next sample
                    sample = self.sample_buffer.popleft()
                    
                    # Add to sequence buffer
                    self.sequence_buffer.append(sample)
                    
                    # Check if we've reached the end of an episode
                    if sample.get('done', False):
                        ...
                        if len(self.sequence_buffer) > 0:
                            ...
                            processed = self._process_sequence(sequence)
                            self.processed_sequences.put(processed)
                        # Clear the buffer to prevent mixing episodes
                        self.sequence_buffer.clear()
                    # Otherwise, process if we have enough frames
                    elif len(self.sequence_buffer) >= self.sequence_length:
                        sequence = list(self.sequence_buffer)
                        processed = self._process_sequence(sequence)
                        self.processed_sequences.put(processed)
                        # Keep the last (sequence_length - 1) frames for the next sequence
                        # to maintain temporal continuity
                        while len(self.sequence_buffer) >= self.sequence_length:
                            self.sequence_buffer.popleft()
                            
            except Exception as e:
                self.logger.error(f"Error in sequence processing: {str(e)}\nTraceback:\n{traceback.format_exc()}")
